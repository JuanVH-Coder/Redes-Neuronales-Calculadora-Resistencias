%% Input image
%close all;
%Imain=imread('res7clc.jpg');
Igray=rgb2gray(Imain);
%imshow(Imain);
%figure;
%imshow(Igray);
%figure;
...
for idx = 1 : length(idxOfResistors)
    res{idx} = imcrop(Imain,statsDefects(idx).BoundingBox);
end
m=1;
for i=1:length(res)
    check=0;
    ...
    if numObjectsRes~=0
        ...
        resimg{m} = imcrop(Imain,statsDefects(i).BoundingBox);
        m=m+1;
        check=1;
    end
    if check ~=1
...
title(['There are ', num2str(length(resimg)), ' resistors in the image!'],'color','w');
